...
class SelectingThenMatching:
    template = Template(
        """Select a record from the following candidates that refers to the same real-world entity as the given record. Answer with the corresponding record number surrounded by "[]".
    ...
    def __call__(self, instance) -> list[bool]:
        ...
        if idx:
            ...
            if 1 <= idx <= len(instance["candidates"]):
                preds[idx - 1] = self.matcher(
                    {
                        "anchor": instance["anchor"],
                        "candidates": [instance["candidates"][idx - 1]],
                    }
                )[0]
...
if __name__ == "__main__":
    ...
    pipeline = SelectingThenMatching()
